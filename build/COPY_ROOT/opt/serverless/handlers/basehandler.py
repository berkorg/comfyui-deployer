...
class BaseHandler:
    ...
    def __init__(self, payload, workflow_json=None):
        ...
        self.request_id = str(self.get_value("request_id", None))
        self.set_prompt()
    ...
    def get_value(self, key, default=None):
        if key not in self.payload and default == None:
            ...
            return default
        elif Network.is_url(self.payload[key]) and not (
            key.startswith("aws_") or key.startswith("webhook_")
        ):
    ...
    def get_url_content(self, url):
        existing_file = Filesystem.find_input_file(
            self.get_input_dir(), Network.get_url_hash(url)
        )
        if existing_file:
            return os.path.basename(existing_file)
        else:
            return os.path.basename(
                Network.download_file(url, self.get_input_dir(), self.request_id)
            )
    ...
    def get_result(self, job_id):
                    ...
                    for item in inner_value:
                        if item.get("type") == "output":
                            ...
                            if os.path.islink(original_path):
                                shutil.copyfile(
                                    os.path.realpath(original_path), new_path
                                )
                            else:
        ...
        self.result["timings"] = {
            ...
            "job_time_total": (
                self.job_time_completed - self.job_time_received
            ).seconds,
        }
